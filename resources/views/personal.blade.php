...
@section('content')
    ...
    <div class="personal">
        <div class="contact">
            @if (Session::has('temporaryUser') && !Auth::user())
                <div class="">
            ...
            @else
                <div>
                    ...
                    <p>{{ Auth::user()->email }}</p>
                    <p>{{ Auth::user()->tel ?: 'Телефон не заполнен' }}
                    </p>
        ...
        @foreach (DB::table('orders')->get() as $item)
            ...
            <div class="order">
                ...
                <div class="goods">
                        @foreach (json_decode($item->properties) as $prop)
                            ...
                            @endif
                            @if ($prop->box->data)
                                <li>Коробка с надписью: "{{ $prop->data->text }}"</li>
                            @endif
                        ...
                        <div class="status {{ $item->status }}">
                            {{-- @switch($item->status)
                            @case('wait') Ожидается оплата @break
                            @case('payed') В печати @break
                            @case('ready') Готов к выдачи @break
                            @case('delivered') В доставке @break
                            @case('closed') Готов @break
                            @endswitch --}}
                        </div>
                ...
                <div class="cost">
                    ...
                    <div>
                        ...
                        <form action="{{ route('payorder') }}" role="form" method="post">
                            ...
                            @endforeach
                            <input class="pay-button" type="submit" value="Олпатить">
                        </form>
                        {{-- <a href="">Оплатить</a> --}}
                        @endif
                    </div>
