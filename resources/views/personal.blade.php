...
<div class="order">
    ...
    <div class="one-good @if ($goodNum > 1) hide @endif" num={{$goodNum}} orderId={{ $item->id }}>
            <li class="head">{{ $prop->product->data }}, {{ $prop->size->data }} - {{ $prop->count->data }} шт.</li>
        ...
        @if ($goodNum > 1)
        <button class="more-button" orderId={{ $item->id }} onclick="moreButtonListener({{ $item->id }})">...</button>
        @endif
...
<script>
    function moreButtonListener (orderId) {
        let goods = document.querySelectorAll('div.one-good[orderid="'+orderId+'"]')
        goods.forEach (good => {
            if (good.getAttribute('num') > 1) {
                if (good.classList.contains('hide')) good.classList.remove ('hide'); else good.classList.add ('hide');
            }
            
        })
    }
    // document.addEventListener('DOMContentLoaded', function () {
    //     let moreButtons = document.querySelectorAll('button.more-button')
